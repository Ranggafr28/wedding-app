        ...
        <div class="flex flex-wrap mt-6 -mx-3">
            <div class="w-full max-w-full px-3 mt-0 lg:w-full lg:flex-none">
                <div
                    ...
                    {{-- filter view --}}
                    <form class="grid grid-cols-4 gap-4 mb-5" action="{{ route($route . '.index') }}" method="GET">
                        <div class="relative">
                            ...
                            <tbody>
                                @foreach ($data as $datas => $item)
                                    <tr class="bg-gray-100 border-b hover:bg-gray-200 text-gray-900">
                                        ...
                                        <td class="px-6 py-4">
                                            {{ $item->product }} <br />[{{ $item->code_product }}]
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $item->fullname }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-center w-32">
                                            <div class="grid grid-cols-2 gap-5">
                                                <a href="{{ route($route . '.edit', [$route => $item->product_id]) }}"
                                                ...
                                                <button onclick="deleteData({{ $item->product_id }})"
                                                ...
                                                <form id="delete-form-{{ $item->product_id }}"
                                                    action="{{ route($route . '.destroy', [$route => $item->product_id]) }}"
                                                    method="post">
                                                    @method('delete')
                                                    @csrf
                                                </form>
